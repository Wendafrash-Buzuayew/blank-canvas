...
@Tag(name = "Media", description = "Uploaded merchant branding images (filesystem storage backend)")
public class MediaController {
    ...
    @Operation(summary = "Fetch an uploaded merchant logo/branding image")
    public ResponseEntity<Resource> getBrandingImage(@PathVariable String filename) throws IOException {
        Path root = Paths.get(uploadDir).toAbsolutePath().normalize();
        Path file = root.resolve("branding").resolve(filename).normalize();
